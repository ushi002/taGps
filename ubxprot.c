...
void ubx_init(void)
{
	...
	messageId = MessageIdGetCfgRxm;
	bufferId = BufferIdGetSetCfgRxm;
	gMessage[messageId].id = (MessageId_e) messageId;
	gMessage[messageId].pMsgBuff = ubx_messages[bufferId];
	gMessage[messageId].confirmType = TypeOfConfirmAck;
	gMessage[messageId].pHead = (UbxPckHeader_s *) gMessage[messageId].pMsgBuff;
...
const Message_s * ubx_get_msg(MessageId_e msgId)
{
	...
	case MessageIdGetCfgRxm:
		ubx_get_cfgrxm(retVal->pMsgBuff);
		break;
	case MessageIdSetCfgRxm:
		ubx_set_cfgrxm(retVal->pMsgBuff);
		break;
	default:
...
U16 ubx_set_cfgrxm(U8 * msg)
{
    ...
    pBody = (UbxCfgRxm_s *) (msg + sizeof(UbxPckHeader_s));
